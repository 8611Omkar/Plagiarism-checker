...
def google_search(query):
    """Fetch search results from Google for a given query, including URLs."""
...
#     results = []
#     for g in soup.find_all('div', class_='tF2Cxc'):  # Class name might vary; inspect Google HTML structure
#         title = g.find('h3').get_text() if g.find('h3') else "No Title"
#         link = g.find('a')['href'] if g.find('a') else "No URL"
...
def detect_ai_content(text):
    ...
    try:
            ...
            print("API Response:", data)  # Optional; remove in production
    
            # Extract source information (adjust based on actual API response structure)
            is_ai_content = data.get("isAIContent", False)  # Whether content is AI-generated
            confidence = data.get("confidence", 0)  # Confidence level (if provided)
            source = data.get("source", "Unknown Source")  # Source or model used for detection
